...
import me.anno.remcraft.block.BlockRegistry.Leaves
import me.anno.remcraft.block.BlockRegistry.Log
...
class TreeDecorator(density: Float = 0.1f, seed: Long = 5123L) :
    ...
    override fun decorate(chunk: Chunk, lx: Int, ly: Int, lz: Int) {
        ...
        for (dz in -2..2) {
            for (dx in -2..2) {
                chunk.setBlockIfAir(lx + dx, cy - 1, lz + dz, Leaves)
                chunk.setBlockIfAir(lx + dx, cy, lz + dz, Leaves)
            }
        }
        for (dz in -1..1) {
            for (dx in -1..1) {
        ...
        chunk.setBlockIfAir(lx, cy + 2, lz, Leaves)
        chunk.setBlockIfAir(lx + 1, cy + 2, lz, Leaves)
        chunk.setBlockIfAir(lx - 1, cy + 2, lz, Leaves)
        chunk.setBlockIfAir(lx, cy + 2, lz + 1, Leaves)
        chunk.setBlockIfAir(lx, cy + 2, lz - 1, Leaves)
        // place stump
        for (i in 0 until stump) {
            chunk.setBlockIfAir(lx, ly + i, lz, Log)
        }
    }
